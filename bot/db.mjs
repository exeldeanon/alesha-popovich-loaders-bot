...
export class BotDatabase {
  ...
  listActiveOrders({city='',region='',userId='',limit=20}={}){
    ...
    if(region&&target)return this.db.prepare(`SELECT o.*,(SELECT COUNT(*) FROM shifts s WHERE s.order_id=o.id AND s.status NOT IN ('cancelled')) AS assigned_count FROM orders o WHERE o.status='active' AND o.region=? AND (o.target_user_id='' OR o.target_user_id=?) ORDER BY o.starts_at LIMIT ?`).all(region,target,limit);
    if(city&&target)return this.db.prepare(`SELECT o.*,(SELECT COUNT(*) FROM shifts s WHERE s.order_id=o.id AND s.status NOT IN ('cancelled')) AS assigned_count FROM orders o WHERE o.status='active' AND lower(o.city)=lower(?) AND (o.target_user_id='' OR o.target_user_id=?) ORDER BY o.starts_at LIMIT ?`).all(city,target,limit);
    if(target)return this.db.prepare(`SELECT o.*,(SELECT COUNT(*) FROM shifts s WHERE s.order_id=o.id AND s.status NOT IN ('cancelled')) AS assigned_count FROM orders o WHERE o.status='active' AND (o.target_user_id='' OR o.target_user_id=?) ORDER BY o.starts_at LIMIT ?`).all(target,limit);
    if(region)return this.db.prepare(`SELECT o.*,(SELECT COUNT(*) FROM shifts s WHERE s.order_id=o.id AND s.status NOT IN ('cancelled')) AS assigned_count FROM orders o WHERE o.status='active' AND o.region=? AND o.target_user_id='' ORDER BY o.starts_at LIMIT ?`).all(region,limit);
    if(city)return this.db.prepare(`SELECT o.*,(SELECT COUNT(*) FROM shifts s WHERE s.order_id=o.id AND s.status NOT IN ('cancelled')) AS assigned_count FROM orders o WHERE o.status='active' AND lower(o.city)=lower(?) AND o.target_user_id='' ORDER BY o.starts_at LIMIT ?`).all(city,limit);
    return this.db.prepare(`SELECT o.*,(SELECT COUNT(*) FROM shifts s WHERE s.order_id=o.id AND s.status NOT IN ('cancelled')) AS assigned_count FROM orders o WHERE o.status='active' AND o.target_user_id='' ORDER BY o.starts_at LIMIT ?`).all(limit);
  }
